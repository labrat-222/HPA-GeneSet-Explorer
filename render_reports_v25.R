############################################################
## Render Generalized report.Rmd for AD, DLB, FTD (v25)
...
diseases <- list(
  AD  = list(gwas = "gwas_list_AD.RDS",
             hpa  = paste0("HPA_data_AD_v25_",  date_str, ".RDS")),
  DLB = list(gwas = "gwas_list_DLB.RDS",
             hpa  = paste0("HPA_data_DLB_v25_", date_str, ".RDS")),
  FTD = list(gwas = "gwas_list_FTD.RDS",
             hpa  = paste0("HPA_data_FTD_v25_", date_str, ".RDS"))
)
...
for (disease_name in names(diseases)) {
  ...
  out_pdf <- file.path(output_dir,
                       paste0("report_", disease_name, "_v25_", date_str, ".pdf"))

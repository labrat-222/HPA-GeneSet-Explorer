...
#you can adjust p threshold here
# Plot for fisher_result_tissue
plot_lollipop(fisher_result = fisher_result_tissue,
              p_value=fisher_result_tissue$fisher_p,
              p_threshold = 0.05, 
              title = "Fold Enrichment Across Tissue Clusters (p_raw<0.05)")

# Plot for fisher_result_SC
plot_lollipop(fisher_result = fisher_result_SC, 
              p_threshold = 0.05, 
              p_value=fisher_result_SC$fisher_p,
              title = "Fold Enrichment Across Single Cell Clusters (p_raw<0.05)")


# Plot for fisher_result_brain
plot_lollipop(fisher_result = fisher_result_brain, 
              p_threshold = 0.05, 
               p_value=fisher_result_brain$fisher_p,
              title = "Fold Enrichment Across Brain Clusters (p_raw<0.05")
```

```{r echo=FALSE, fig.height=10, fig.width=9,warning=FALSE}
plot_lollipop(fisher_result = fisher_result_tissue,
              p_value =fisher_result_tissue$fisher_p_adj,
              p_threshold = 0.1, 
              title = "Fold Enrichment Across Tissue Clusters (p_adj<0.1)")

plot_lollipop(fisher_result = fisher_result_SC, 
              p_threshold = 0.1, 
              p_value=fisher_result_SC$fisher_p_adj,
              title = "Fold Enrichment Across Single Cell Clusters(p_adj<0.1)")

plot_lollipop(fisher_result = fisher_result_brain, 
              p_threshold = 0.1, 
               p_value=fisher_result_brain$fisher_p_adj,
              title = "Fold Enrichment Across Brain Clusters (p_adj<0.1)")
...
```{r,echo=FALSE}
ggplot(fisher_result_tissue,
       aes(fold_enrichment, -log(fisher_p_adj))) +
  
  geom_point(color = "grey70", alpha = 0.6) +
  
  geom_point(data = subset(fisher_result_tissue, fisher_p < 0.05),
             color = "orange", size = 2) +
  geom_point(data = subset(fisher_result_tissue, fisher_p_adj < 0.1),
             color = "red", size = 2) +
  
  geom_text(data = subset(fisher_result_tissue, fisher_p < 0.05),
            aes(label = UMAP.Cluster.tissue),
            vjust = -0.5,
            size = 3) +
   geom_vline(xintercept = 1, linetype = "dashed", color = "skyblue3") +
  ggtitle("Tissue cluster_FTD") 


ggplot(fisher_result_SC,
       aes(fold_enrichment, -log(fisher_p_adj))) +
  
  geom_point(color = "grey70", alpha = 0.6) +
  
  geom_point(data = subset(fisher_result_SC, fisher_p < 0.05),
             color = "orange", size = 2) +
  geom_point(data = subset(fisher_result_SC, fisher_p_adj < 0.1),
             color = "red", size = 2) +
  
  geom_text(data = subset(fisher_result_SC, fisher_p < 0.05),
            aes(label = UMAP.Cluster.singlecell),
            vjust = -0.5,
            size = 3) +
     geom_vline(xintercept = 1, linetype = "dashed", color = "skyblue3") +
  ggtitle("SC cluster_FTD") 

ggplot(fisher_result_brain,
       aes(fold_enrichment, -log(fisher_p_adj))) +
  
  geom_point(color = "grey70", alpha = 0.6) +
  
  geom_point(data = subset(fisher_result_brain, fisher_p < 0.05),
             color = "orange", size = 2) +
  geom_point(data = subset(fisher_result_brain, fisher_p_adj < 0.1),
             color = "red", size = 2) +
  
  geom_text(data = subset(fisher_result_brain, fisher_p < 0.05),
            aes(label = UMAP.Cluster.brain),
            vjust = -0.5) +
    geom_vline(xintercept = 1, linetype = "dashed", color = "skyblue3") +
  ggtitle("Brain cluster_FTD") 
